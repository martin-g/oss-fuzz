...
./gradlew shadowJar -p spring-shell-core/ -x test -x javadoc

CURRENT_VERSION=$(./gradlew properties --no-daemon --console=plain | sed -nr "s/^version:\ (.*)/\1/p")
cp ./spring-shell-core/build/libs/spring-shell-core-$CURRENT_VERSION-all.jar $OUT/spring-shell-core.jar

ALL_JARS="spring-shell-core.jar"
